...
| AC-003 | `/mcp` 接受固定查询参数 `apikey` 中的有效 runtime API Key。 | API | 已创建有效 runtime API Key。 | 访问 `/mcp?apikey=<key>` 的 MCP 客户端可初始化会话并调用 `list_datasources`。 |
...
| AC-005 | `/mcp` 对多个一致的非空来源只校验同一把 API Key。 | API | 已创建有效 runtime API Key。 | 同时发送 Header、`apikey` 和 Bearer，且三个值相同，请求可初始化 MCP 会话。 |
| AC-006 | `/mcp` 拒绝多个不同的非空 API Key 值。 | API | 请求同时携带至少两个不同的 Header、`apikey` 或 Bearer 值。 | 请求在调用 API Key 数据库校验前返回 HTTP 400。 |
| AC-007 | `/mcp` 对没有可用凭证的请求保持现有缺失凭证响应。 | API | 请求不含 API Key Header、非空 `apikey` 或有效 Bearer 凭证。 | 返回 HTTP 401，响应体的 `detail` 为 `Missing API key`。 |
